...
test("native publication creates once, retries unchanged, and skips presentation-only records", async () => {
  ...
  assert.ok(output.indexOf("Published from [The Bridge]") < output.indexOf('{{< discussionbridge mode="from_discourse" >}}'));
  assert.match(output, /DiscussionBridge for Hugo 0\.1\.0-alpha\.8/);
